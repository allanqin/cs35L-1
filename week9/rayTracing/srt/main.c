...
void threadfunction(void *arg)
{
    ...
    for( int px=(int)arg; px<end; ++px )
    {
        ...
        RGB_t temp;
        temp.zero = scaled_color[0];
        temp.one = scaled_color[1];
        temp.two = scaled_color[2];
        image[px*width+py] = temp;
...
int main( int argc, char **argv )
{
    ...
    image = (RGB_t*) malloc(sizeof(RGB_t)*width*height);
    ...
    /* create threads here */ 
    //create start indexes for each thread
    sliceSize = width / nthreads;   //width of each thread's "slice" of the image
    int *starts = (int*)malloc(sizeof(int)*nthreads);
    for (int k = 0; k < nthreads; k++)
    {
        starts[k] = k * sliceSize;
    }
    ...
    for (int k = 0; k < nthreads; k++)
    {
        rs = pthread_join(threadID[k], &retval2);
        if (rs)
        {
   ...
   for (int px = 0; px < width; ++px)
    {
	    for (int py = 0; py < height; ++py)
	    {
		RGB_t toprint = image[px*width+py];
                printf( "%.0f %.0f %.0f\n",
		    toprint.zero, toprint.one, toprint.two );
	    }
	    printf("\n");
    }
